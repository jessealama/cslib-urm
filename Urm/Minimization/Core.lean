...
def extend_inputs {n : ℕ} (inputs : Fin n → ℕ) (y : ℕ) : Fin (n + 1) → ℕ :=
  Fin.snoc inputs y

/-- The predicate that checks if f returns 0, as a partial Bool. -/
def check_zero {n : ℕ} (f : (Fin (n + 1) → ℕ) → Part ℕ) (inputs : Fin n → ℕ) (y : ℕ) : Part Bool :=
  Part.map (· = 0) (f (extend_inputs inputs y))
...
def μ {n : ℕ} (f : (Fin (n + 1) → ℕ) → Part ℕ) (inputs : Fin n → ℕ) : Part ℕ :=
  Nat.rfind (check_zero f inputs)

/-- The function computed by μ, for stating computability theorems. -/
def μFunction {n : ℕ} (f : (Fin (n + 1) → ℕ) → Part ℕ) : (Fin n → ℕ) → Part ℕ :=
  μ f

/-! ## Helper lemmas for check_zero -/

theorem check_zero_true_iff {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ} {y : ℕ} :
    true ∈ check_zero f inputs y ↔ 0 ∈ f (extend_inputs inputs y) := by
  simp only [check_zero, Part.mem_map_iff, eq_comm, true_eq_decide_iff]
  constructor
  · intro ⟨v, hv, hv0⟩; subst hv0; exact hv
  · intro h; exact ⟨0, h, rfl⟩

theorem check_zero_false_iff {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ} {y : ℕ} :
    false ∈ check_zero f inputs y ↔ ∃ v ≠ 0, v ∈ f (extend_inputs inputs y) := by
  simp only [check_zero, Part.mem_map_iff, eq_comm, false_eq_decide_iff, ne_eq]
  constructor
  · intro ⟨v, hv, hv_ne0⟩; exact ⟨v, hv_ne0, hv⟩
  · intro ⟨v, hv_ne0, hv⟩; exact ⟨v, hv, hv_ne0⟩

theorem check_zero_dom_iff {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ} {y : ℕ} :
    (check_zero f inputs y).Dom ↔ (f (extend_inputs inputs y)).Dom := by
  simp only [check_zero, Part.dom_iff_mem, Part.mem_map_iff]
  constructor
...
theorem μ_dom_iff {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ} :
    (μ f inputs).Dom ↔
      ∃ y, (f (extend_inputs inputs y) = Part.some 0) ∧
           ∀ y' < y, (f (extend_inputs inputs y')).Dom := by
  unfold μ
  rw [Nat.rfind_dom]
  constructor
  · intro ⟨y, htrue, hdom⟩
    refine ⟨y, ?_, ?_⟩
    · rw [check_zero_true_iff] at htrue
      exact Part.eq_some_iff.mpr htrue
    · intro y' hy'
      rw [← check_zero_dom_iff]
      exact hdom hy'
  · intro ⟨y, hfy, hdom⟩
    refine ⟨y, ?_, ?_⟩
    · rw [check_zero_true_iff]
...
theorem μ_spec {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ}
    (h : (μ f inputs).Dom) : f (extend_inputs inputs ((μ f inputs).get h)) = Part.some 0 := by
  unfold μ at h ⊢
  have hspec := Nat.rfind_spec (Part.get_mem h)
  rw [check_zero_true_iff] at hspec
...
theorem μ_min {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ}
    ...
    ∃ v, f (extend_inputs inputs y') = Part.some v ∧ v ≠ 0 := by
  unfold μ at h hy'
  have hmin := Nat.rfind_min (Part.get_mem h) hy'
  rw [check_zero_false_iff] at hmin
...
theorem μ_dom_below {n : ℕ} {f : (Fin (n + 1) → ℕ) → Part ℕ} {inputs : Fin n → ℕ}
    (h : (μ f inputs).Dom) {y' : ℕ} (hy' : y' < (μ f inputs).get h) :
    (f (extend_inputs inputs y')).Dom := by
  obtain ⟨v, hv, _⟩ := μ_min h hy'
  rw [Part.eq_some_iff] at hv
  exact Part.dom_iff_mem.mpr ⟨v, hv⟩

/-- extend_inputs at index i < n returns the original input. -/
@[simp] theorem extend_inputs_castSucc {n : ℕ} (inputs : Fin n → ℕ) (y : ℕ) (i : Fin n) :
    extend_inputs inputs y (Fin.castSucc i) = inputs i := by
  simp [extend_inputs, Fin.snoc]

/-- extend_inputs at index n returns y. -/
@[simp] theorem extend_inputs_last {n : ℕ} (inputs : Fin n → ℕ) (y : ℕ) :
    extend_inputs inputs y (Fin.last n) = y := by
  simp [extend_inputs, Fin.snoc]
