...
@[simp] theorem instr_0 : add_program[0]? = some (Instr.Z 2) := rfl
@[simp] theorem instr_1 : add_program[1]? = some (Instr.J 2 1 5) := rfl
@[simp] theorem instr_2 : add_program[2]? = some (Instr.S 0) := rfl
@[simp] theorem instr_3 : add_program[3]? = some (Instr.S 2) := rfl
@[simp] theorem instr_4 : add_program[4]? = some (Instr.J 0 0 1) := rfl
@[simp] theorem length_eq : add_program.length = 5 := rfl

/-- The initial instruction (Z 2) takes us from pc=0 to pc=1 with R2=0. -/
theorem step_init (s : State) :
    Step add_program ⟨0, s⟩ ⟨1, s.write 2 0⟩ :=
  Step.zero instr_0

/-- When counter equals y (R2 = R1), we exit the loop by jumping to pc=5. -/
theorem step_exit (s : State) (heq : s.read 2 = s.read 1) :
    Step add_program ⟨1, s⟩ ⟨5, s⟩ :=
...
theorem loop_iteration (s : State) (k y : ℕ)
    ...
    ∃ s', Steps add_program ⟨1, s⟩ ⟨1, s'⟩ ∧
          s'.read 0 = s.read 0 + 1 ∧
          s'.read 1 = y ∧
          s'.read 2 = k + 1 := by
  have hne : s.read 2 ≠ s.read 1 := by omega
  have h1 : Step add_program ⟨1, s⟩ ⟨2, s⟩ := step_continue s hne
  let s1 := s.write 0 (s.read 0 + 1)
  have h2 : Step add_program ⟨2, s⟩ ⟨3, s1⟩ := step_inc_r0 s
  let s2 := s1.write 2 (s1.read 2 + 1)
  have h3 : Step add_program ⟨3, s1⟩ ⟨4, s2⟩ := step_inc_r2 s1
  have h4 : Step add_program ⟨4, s2⟩ ⟨1, s2⟩ := step_jump_back s2
...
theorem loop_invariant (x y k : ℕ) (hk : k ≤ y) :
    ∃ s, Steps add_program ⟨1, (State.of_inputs [x, y]).write 2 0⟩ ⟨1, s⟩ ∧
         s.read 0 = x + k ∧
         s.read 1 = y ∧
         s.read 2 = k := by
  induction k with
  | zero =>
    refine ⟨_, Steps.refl _, ?_, ?_, ?_⟩ <;>
      simp [State.write, State.read, State.of_inputs, Function.update_of_ne, Function.update_self]
  | succ k' ih =>
...
theorem exits_when_done (s : State) (h : s.read 2 = s.read 1) :
    Steps add_program ⟨1, s⟩ ⟨5, s⟩ := by
  have hstep := step_exit s h
  aesop_steps

/-- Configuration at pc=5 is halted. -/
theorem halted_at_5 (s : State) : (⟨5, s⟩ : Config).is_halted add_program := by
  simp [Config.is_halted, length_eq]

/-- The full execution from initial state to halted state. -/
theorem full_execution (x y : ℕ) :
    ∃ s, Steps add_program (Config.init [x, y]) ⟨5, s⟩ ∧
         s.read 0 = x + y ∧
         (⟨5, s⟩ : Config).is_halted add_program := by
  let s0 := State.of_inputs [x, y]
  have h_init : Step add_program ⟨0, s0⟩ ⟨1, s0.write 2 0⟩ := step_init s0
...
theorem add_computable : URMComputable 2 (fun xy => Part.some (xy 0 + xy 1)) := by
  ...
  · simp only [Part.some_dom, iff_true]
    rw [h_ofFn]
    obtain ⟨s, hsteps, _, hhalted⟩ := add_program.full_execution x y
    exact ⟨⟨5, s⟩, hsteps, hhalted⟩
  · intro hHalts hDom
    obtain ⟨s, hsteps, hr0, hhalted⟩ := add_program.full_execution x y
    have hsteps' : Steps add_program (Config.init (List.ofFn inputs)) ⟨5, s⟩ := by
...
namespace pred_program

-- Helper lemmas about instruction lookup
@[simp] theorem instr_0 : pred_program[0]? = some (Instr.Z 1) := rfl
@[simp] theorem instr_1 : pred_program[1]? = some (Instr.J 0 1 9) := rfl
@[simp] theorem instr_2 : pred_program[2]? = some (Instr.Z 2) := rfl
@[simp] theorem instr_3 : pred_program[3]? = some (Instr.S 1) := rfl
@[simp] theorem instr_4 : pred_program[4]? = some (Instr.J 0 1 8) := rfl
@[simp] theorem instr_5 : pred_program[5]? = some (Instr.S 2) := rfl
@[simp] theorem instr_6 : pred_program[6]? = some (Instr.S 1) := rfl
@[simp] theorem instr_7 : pred_program[7]? = some (Instr.J 0 0 4) := rfl
@[simp] theorem instr_8 : pred_program[8]? = some (Instr.T 2 0) := rfl
@[simp] theorem length_eq : pred_program.length = 9 := rfl

/-- Clear R1 for zero check (pc 0 → 1). -/
theorem step_clear_r1 (s : State) :
    Step pred_program ⟨0, s⟩ ⟨1, s.write 1 0⟩ :=
  Step.zero instr_0

/-- When R0 = 0 (= R1 after clear), jump to halt (pc 1 → 9). -/
theorem step_zero_exit (s : State) (heq : s.read 0 = s.read 1) :
    Step pred_program ⟨1, s⟩ ⟨9, s⟩ :=
  Step.jump_eq instr_1 heq

/-- When R0 ≠ 0, continue to main loop setup (pc 1 → 2). -/
theorem step_nonzero_continue (s : State) (hne : s.read 0 ≠ s.read 1) :
    Step pred_program ⟨1, s⟩ ⟨2, s⟩ :=
  Step.jump_ne instr_1 hne

/-- Clear R2 for result tracking (pc 2 → 3). -/
theorem step_clear_r2 (s : State) :
    Step pred_program ⟨2, s⟩ ⟨3, s.write 2 0⟩ :=
  Step.zero instr_2

/-- Initialize R1 to 1 (pc 3 → 4). -/
theorem step_init_r1 (s : State) :
    Step pred_program ⟨3, s⟩ ⟨4, s.write 1 (s.read 1 + 1)⟩ :=
  Step.succ instr_3

/-- When R0 = R1, jump to copy result (pc 4 → 8). -/
theorem step_found_pred (s : State) (heq : s.read 0 = s.read 1) :
    Step pred_program ⟨4, s⟩ ⟨8, s⟩ :=
  Step.jump_eq instr_4 heq

/-- When R0 ≠ R1, continue loop (pc 4 → 5). -/
theorem step_continue_loop (s : State) (hne : s.read 0 ≠ s.read 1) :
    Step pred_program ⟨4, s⟩ ⟨5, s⟩ :=
  Step.jump_ne instr_4 hne

/-- Increment R2 (pc 5 → 6). -/
theorem step_inc_r2 (s : State) :
    Step pred_program ⟨5, s⟩ ⟨6, s.write 2 (s.read 2 + 1)⟩ :=
  Step.succ instr_5

/-- Increment R1 (pc 6 → 7). -/
theorem step_inc_r1 (s : State) :
    Step pred_program ⟨6, s⟩ ⟨7, s.write 1 (s.read 1 + 1)⟩ :=
  Step.succ instr_6

/-- Unconditional jump back to loop check (pc 7 → 4). -/
theorem step_jump_back (s : State) :
    Step pred_program ⟨7, s⟩ ⟨4, s⟩ :=
  Step.jump_eq instr_7 rfl

/-- Copy R2 to R0 (pc 8 → 9). -/
theorem step_copy_result (s : State) :
    Step pred_program ⟨8, s⟩ ⟨9, s.write 0 (s.read 2)⟩ :=
  Step.trans instr_8

/-- Configuration at pc=9 is halted. -/
theorem halted_at_9 (s : State) : (⟨9, s⟩ : Config).is_halted pred_program := by
  simp [Config.is_halted, length_eq]

/-- Full execution for n = 0: immediately exits with R0 = 0. -/
theorem zero_execution :
    ∃ s, Steps pred_program (Config.init [0]) ⟨9, s⟩ ∧
         s.read 0 = 0 ∧
         (⟨9, s⟩ : Config).is_halted pred_program := by
  let s0 := State.of_inputs [0]
  have hs0_r0 : s0 0 = 0 := rfl
  have hs0_r1 : s0 1 = 0 := rfl
  let s1 := s0.write 1 0
  have h1 : Step pred_program ⟨0, s0⟩ ⟨1, s1⟩ := step_clear_r1 s0
  ...
  have h2 : Step pred_program ⟨1, s1⟩ ⟨9, s1⟩ := step_zero_exit s1 heq
  use s1
...
theorem loop_iteration (s : State) (n k : ℕ)
    ...
    (hlt : k + 1 < n) :
    ∃ s', Steps pred_program ⟨4, s⟩ ⟨4, s'⟩ ∧
          s'.read 0 = n ∧
          s'.read 1 = k + 2 ∧
          s'.read 2 = k + 1 := by
  have hne : s.read 0 ≠ s.read 1 := by omega
  have h1 : Step pred_program ⟨4, s⟩ ⟨5, s⟩ := step_continue_loop s hne
  let s1 := s.write 2 (s.read 2 + 1)
  have h2 : Step pred_program ⟨5, s⟩ ⟨6, s1⟩ := step_inc_r2 s
  let s2 := s1.write 1 (s1.read 1 + 1)
  have h3 : Step pred_program ⟨6, s1⟩ ⟨7, s2⟩ := step_inc_r1 s1
  have h4 : Step pred_program ⟨7, s2⟩ ⟨4, s2⟩ := step_jump_back s2
  refine ⟨s2, by aesop_steps, ?_, ?_, ?_⟩
...
theorem loop_invariant (n k : ℕ) (hk : k < n) :
    ∃ s, Steps pred_program ⟨4, (State.of_inputs [n]).write 1 0 |>.write 2 0 |>.write 1 1⟩ ⟨4, s⟩ ∧
         s.read 0 = n ∧
         s.read 1 = k + 1 ∧
         s.read 2 = k := by
  induction k with
  | zero =>
    let s_init := (State.of_inputs [n]).write 1 0 |>.write 2 0 |>.write 1 1
    refine ⟨s_init, Steps.refl _, ?_, ?_, ?_⟩
    · simp [s_init, State.read, State.write, State.of_inputs]
...
theorem loop_exit (s : State) (n : ℕ)
    ...
    ∃ s', Steps pred_program ⟨4, s⟩ ⟨9, s'⟩ ∧
          s'.read 0 = n - 1 ∧
          (⟨9, s'⟩ : Config).is_halted pred_program := by
  have heq : s.read 0 = s.read 1 := by rw [hr0, hr1]
  have h1 : Step pred_program ⟨4, s⟩ ⟨8, s⟩ := step_found_pred s heq
  let s' := s.write 0 (s.read 2)
  have h2 : Step pred_program ⟨8, s⟩ ⟨9, s'⟩ := step_copy_result s
  use s'
...
theorem nonzero_execution (n : ℕ) (hn : 0 < n) :
    ∃ s, Steps pred_program (Config.init [n]) ⟨9, s⟩ ∧
         s.read 0 = n - 1 ∧
         (⟨9, s⟩ : Config).is_halted pred_program := by
  ...
  have hs0_r1 : s0 1 = 0 := rfl
  let s1 := s0.write 1 0
  have h_step0 : Step pred_program ⟨0, s0⟩ ⟨1, s1⟩ := step_clear_r1 s0
  ...
  have h_step1 : Step pred_program ⟨1, s1⟩ ⟨2, s1⟩ := step_nonzero_continue s1 hne1
  let s2 := s1.write 2 0
  have h_step2 : Step pred_program ⟨2, s1⟩ ⟨3, s2⟩ := step_clear_r2 s1
  let s3 := s2.write 1 (s2.read 1 + 1)
  have h_step3 : Step pred_program ⟨3, s2⟩ ⟨4, s3⟩ := step_init_r1 s2
  have hs3_eq : s3 = ((((State.of_inputs [n]).write 1 0).write 2 0).write 1 1) := by
  ...
  have h_loop_steps' : Steps pred_program ⟨4, s3⟩ ⟨4, s_loop⟩ := by
...
theorem pred_computable : URMComputable 1 (fun x => Part.some (x 0 - 1)) := by
  ...
  · simp only [Part.some_dom, iff_true]
    ...
    | inl hz =>
      rw [h_ofFn, hz]
      obtain ⟨s, hsteps, _, hhalted⟩ := pred_program.zero_execution
      exact ⟨⟨9, s⟩, hsteps, hhalted⟩
    | inr hpos =>
      rw [h_ofFn]
      obtain ⟨s, hsteps, _, hhalted⟩ := pred_program.nonzero_execution n hpos
      exact ⟨⟨9, s⟩, hsteps, hhalted⟩
  · intro hHalts hDom
    obtain ⟨hsteps_chosen, hhalted_chosen⟩ := Classical.choose_spec hHalts
    cases Nat.eq_zero_or_pos n with
    | inl hz =>
      obtain ⟨s, hsteps, hr0, hhalted⟩ := pred_program.zero_execution
      have hsteps' : Steps pred_program (Config.init (List.ofFn inputs)) ⟨9, s⟩ := by
        simp only [h_ofFn, hz]; exact hsteps
      have heq := Steps.eq_of_halts hsteps_chosen hhalted_chosen hsteps' hhalted
      simp only [Result, heq, State.output, Part.get_some, State.read] at hr0 ⊢
      omega
    | inr hpos =>
      obtain ⟨s, hsteps, hr0, hhalted⟩ := pred_program.nonzero_execution n hpos
      have hsteps' : Steps pred_program (Config.init (List.ofFn inputs)) ⟨9, s⟩ := by
        simp only [h_ofFn]; exact hsteps
      have heq := Steps.eq_of_halts hsteps_chosen hhalted_chosen hsteps' hhalted
      simp only [Result, heq, State.output, Part.get_some]
...
private theorem sub_pr_step_eq (inputs : Fin 3 → ℕ) :
    comp_function 1 3 (fun x => Part.some (x 0 - 1)) (fun _ => fun y => Part.some (y 2)) inputs =
    Part.some (inputs 2 - 1) := by
  simp only [comp_function, Part.sequence, Part.bind_some, Fin.isValue]
...
private theorem sub_pr_eq (m n : ℕ) :
  ...
  | succ k ih =>
    ...
    have h : extend_inputs_for_g (fun _ : Fin 1 => m) k (m - k) 2 = m - k := by
      simp only [extend_inputs_for_g, h2_eq, Fin.snoc_last]
...
private theorem mul_pr_step_eq (inputs : Fin 3 → ℕ) :
    comp_function 2 3 (fun xy => Part.some (xy 0 + xy 1)) mulStepGs inputs =
  ...
  simp only [comp_function, Part.sequence, h0, h1, Part.bind_some, Part.map_some]; rfl
...
private theorem mul_pr_eq (x y : ℕ) :
  ...
  | succ k ih =>
    ...
    rw [ih]
    simp only [Part.bind_some]
    have h0 : extend_inputs_for_g (fun _ : Fin 1 => x) k (x * k) 0 = x := by
      simp only [extend_inputs_for_g, Fin.snoc]; rfl
    have h2_eq : (2 : Fin 3) = Fin.last 2 := rfl
    have h2 : extend_inputs_for_g (fun _ : Fin 1 => x) k (x * k) 2 = x * k := by
      simp only [extend_inputs_for_g, h2_eq, Fin.snoc_last]
