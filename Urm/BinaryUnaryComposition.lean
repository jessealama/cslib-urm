...
with m = 2 (binary f) and n = 1 (unary g₁, g₂). -/
theorem URMComputableSF.comp_binary_unary
  ...
  -- Define gs as the pair (g1, g2) indexed by Fin 2
  let gs : Fin 2 → (Fin 1 → ℕ) → Part ℕ := fun i => if i.val = 0 then g1 else g2
  -- Prove computability for each component
  have hgs : ∀ i, URMComputableSF 1 (gs i) := fun i => by
    simp only [gs]
    by_cases hi : i.val = 0 <;> simp [hi]
    · exact hg1
    · exact hg2
  -- Apply general composition
  convert URMComputableSF.comp_general hf hgs using 2
