...
theorem Steps.straight_line_at_offset {p P : Program} {c c' : Config} (k : ℕ)
    (hsl : p.is_straight_line = true)
    (hembed : ∀ i, i < p.length → P[k + i]? = p[i]?)
    (hsteps : Steps p c c') :
...
/-- A step in p lifts to a step in P when p.shift_jumps(offset) is embedded at offset in P. -/
theorem Step.shift_jumps_at_offset {p P : Program} {c c' : Config} (offset : ℕ)
    (hembed : ∀ i, i < p.length → P[offset + i]? = (p.shift_jumps offset)[i]?)
    (hstep : Step p c c')
    (hpc_bound : c.pc < p.length) :
    Step P ⟨offset + c.pc, c.state⟩ ⟨offset + c'.pc, c'.state⟩ := by
  have hinstr_eq : P[offset + c.pc]? = (p.shift_jumps offset)[c.pc]? :=
...
/-- Steps in p lift to steps in P when p.shift_jumps(offset) is embedded at offset in P. -/
theorem Steps.shift_jumps_at_offset {p P : Program} {c c' : Config} (offset : ℕ)
    (hembed : ∀ i, i < p.length → P[offset + i]? = (p.shift_jumps offset)[i]?)
    (hsteps : Steps p c c') :
  ...
  | @head a b hstep _ ih =>
    have hpc_bound := Step.pc_lt_length hstep
    have hstep' := Step.shift_jumps_at_offset offset hembed hstep hpc_bound
    exact Relation.ReflTransGen.head hstep' ih
